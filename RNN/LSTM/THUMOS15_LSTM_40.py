...
# region NumPy Test
data = sio.loadmat('../THUMOS_ERNN/D14_P40_PY.mat')

nSeq = data['trFEAT_CAT'][0].shape[0]

X = np.ndarray((nSeq, 40, 909), dtype=theano.config.floatX)
y = np.ndarray((nSeq, 40), dtype=np.int32)
M = np.ndarray((nSeq, 40), dtype=theano.config.floatX)
for i in range(nSeq):
...
X1 = np.ndarray((nSeq1, 40, 909), dtype=theano.config.floatX)
y1 = np.ndarray((nSeq1, 40), dtype=np.int32)
M1 = np.ndarray((nSeq1, 40), dtype=theano.config.floatX)
...
# endregion
sio.savemat('LSTM_pram40.mat', {'lstm_W':v1[0], 'lstm_U':v1[1], 'lstm_b':v1[2], 'W':v1[3], 'b':v1[4], 'pred40':output_pred,
