...
for i = 1:size(VAL_TM_FEAT, 1)
    ...
    T1 = VAL_TM_FEAT{i, 2}';    
%     T1 = bsxfun(@rdivide, T1, sqrt(sum(T1 .^2, 2)));
    T2 = VAL_P5TM_FEAT{i, 2}';    
%     T2 = bsxfun(@rdivide, T2, sqrt(sum(T2 .^2, 2)));
    
    T = [4*T1, T2(1:size(T1, 1), :)];
...
for i = 1:length(FEAT_TM)
    ...
    if length(tmp_lbl) > SEG_LEN/4
        len = length(tmp_lbl);
        FEAT_SEG{i}{nSeg+1} = [tmp_feat(1:len, :); zeros( SEG_LEN1 - len, 909*2)];
